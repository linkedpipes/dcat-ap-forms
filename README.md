...
You can generate a prefilled form using HTTP POST with `application/x-www-form-urlencoded` content type.
The request must contain `formData` field which contains the registration record.
In addition, you can provide `userData` field that can be used with `returnUrl` URL query.

An example of the `formData`:
```json
{
...
```
In the HTML response we can see `window.serverPostData.formData` or `window.serverPostData.userData` set to the posted content.
The client will load this data in a same way as when importing from URL.

While the simple example above is sufficient for dataset delete, as only basic information is utilized, you may need to provide more information when using dataset or catalog edit available at `./registrace-datové-sady` or `./dataset-registration`.
...
The request is using `application/x-www-form-urlencoded` content type with the `formData` and optionally `userData` data field.

We can even create a nice loop, using following URL `./registrace-datové-sady?returnUrl=./odstranění-datové-sady`.
In this case the user is navigated to an empty registration form.
Once the form is filled, the user can submit the form using POST to the dataset delete form.
